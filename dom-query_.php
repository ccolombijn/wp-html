...
{ // http://nl1.php.net/manual/en/class.domdocument.php
  ...
  private function get_element( $dom, $element, $val )
  {
    if ( strpos( $element, '#' ) !== false || strpos( $element, '.' ) !== false  ) // element#id
    { // selecteer .class of #id van element

    } else { // selecteer op tag van element

      $dom_nodelist = get_tag_( $dom, $element );

    }
    foreach( $dom_nodelist as $element )
    { // loop door alle elementen en vervang met opgegeven inhoud
  ...
  { // http://ca.php.net/manual/en/domdocument.getelementbyid.php
    $xpath = new DomXPath($dom);
  ...
  private function element( $dom, $element )
  {
    ...
    {
      $el = explode( '#',$element );
      $element = $this->get_id_( $dom, $el[1] );
    }
    elseif ( strpos( $element, '.' ) !== false ) // element.class
    {
      $el = explode( '.',$element );
      $element = $this->get_class_( $dom, $el[1] );
    }
    else {
  ...
  { // http://php.net/manual/en/domelement.setattribute.php
      ...
      $dom_nodelist = $this->get_element( $dom, $element, $val );
